...
const TODOS = [
  { id: "1", title: "อ่านสไลด์สัปดาห์ 5", done: true, priority: "high" },
...
todoRouter.get("/health", (req, res) => { // แก้ไข
  res.json({ status: "ok" });
});

todoRouter.get("/", (req, res) => { // ดึงทั้งหมด
  res.json(TODOS.map((t) => ({ ...t })));
});

todoRouter.get("/:id", (req, res) => { // แก้ไข
